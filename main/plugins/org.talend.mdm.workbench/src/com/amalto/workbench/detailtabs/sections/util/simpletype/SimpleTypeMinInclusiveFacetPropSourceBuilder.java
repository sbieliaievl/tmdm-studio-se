...
public class SimpleTypeMinInclusiveFacetPropSourceBuilder extends SimpleTypeFacetPropSourceBuilder {
    ...
    protected IPropertySource<?> doCreatePropSource(XSDSimpleTypeDefinition simpleType, Composite cellEditorParent,
        ...
        if (Util.isDate(simpleType)) {
            return new SimpleTypeFacetDatePropertySource(cellEditorParent, IConstants.SIMPLETYPE_FACETNAME_MININCLUSIVE,
    ...
    protected Object getSourceFacetValue(XSDSimpleTypeDefinition simpleType) {
        ...
        if (Util.isDate(simpleType) || Util.isDateTime(simpleType) || Util.isTime(simpleType)) {
            return SimpleTypeFacetValueExtractor.getStringFacetValue(simpleType.getMinInclusiveFacet());
        }
